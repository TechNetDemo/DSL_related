        ...
        stage('DEV') { 
            input{ 
                ...
                parameters{
                    ...
                    string(name: 'db_password', defaultValue: '')
                }  
            }
            steps {
                input message: 'Start DEV Deployment? (by approver)', submitter: 'admin,dev_admin'
                build job: '/A.B/DEV/A.B_DEV_Pipeline', parameters: [string(name: 'artifact_version', value: artifact_version), string(name: 'upstream', value: JOB_NAME), string(name: 'db_username', value: db_username),string(name: 'db_password', value: db_password)]
            }
        }
        stage('SIT') { 
            input{ 
                ...
                parameters{
                    string(name: 'db_username', defaultValue: 'root', description: 'account for WebApp connect to MySQL')
                    string(name: 'db_password', defaultValue: '',description: 'password of the account')
                }  
            }
            steps {
